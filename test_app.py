...
class CastingAgencyCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = 'postgres://{}:{}@{}/{}'.format('postgres'
                , '123', 'localhost:5432', self.database_name)
        ...
        self.assistance_header = \
            {'Authorization': self.assistance_token}
    ...
    def test_delete_actor(self):
        res = self.client().delete('/actors/18',
                                   headers=self.producer_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_patch_actor(self):
        res = self.client().patch('/actors/13', json={'age': 22},
                                  headers=self.producer_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_patch_movie(self):
        res = self.client().patch('/movies/1',
                                  json={'title': 'law abiding citizen'
                                  }, headers=self.producer_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
